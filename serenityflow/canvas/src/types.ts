...
interface SFTypeInfo {
    color: string;
    name: string;
}

const SF_TYPES: Record<string, SFTypeInfo> = {
...
const SF_CATEGORY_COLORS: Record<string, string> = {
    'sampling':      '#4a3a6a',
    'conditioning':  '#3a4a3a',
    'loaders':       '#3a3a5a',
    'latent':        '#4a3a3a',
...
function getTypeColor(typeName: string | null | undefined): string {
    if (!typeName) return '#808080';
    const key: string = String(typeName).toUpperCase().replace(/ /g, '_');
    const t: SFTypeInfo = SF_TYPES[key] || SF_TYPES['*'];
    return t.color;
}

function getCategoryColor(category: string | null | undefined): string {
    if (!category) return SF_CATEGORY_COLORS._default;
    const base: string = category.split('/')[0].toLowerCase();
