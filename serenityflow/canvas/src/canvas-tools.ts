...
interface CanvasToolContext {
    ...
    addLayer(name: string, type: string): CanvasLayer;
...
interface CanvasTool {
    ...
    onActivate?(ctx: CanvasToolContext): void;
    onDeactivate?(ctx: CanvasToolContext): void;
    onMouseDown?(ctx: CanvasToolContext, e: Konva.KonvaEventObject<MouseEvent>): void;
    onMouseMove?(ctx: CanvasToolContext, pos: { x: number; y: number }): void;
    onMouseUp?(ctx: CanvasToolContext): void;
    onKeyDown?(ctx: CanvasToolContext, e: KeyboardEvent): void;
}
...
var CanvasTools = (function() {
    ...
    var _drawing = false;
    var _currentLine: Konva.Line | null = null;
    var _drawScheduled = false;

    // Rect tool state
    var _rectStart: { x: number; y: number } | null = null;
    var _rectPreview: Konva.Rect | null = null;

    // Gradient tool state
    var _gradStart: { x: number; y: number } | null = null;
    var _gradPreview: Konva.Line | null = null;

    // Move tool state
    var _moveTarget: Konva.Node | null = null;
    var _moveStart: { x: number; y: number } | null = null;
    var _moveNodeStart: { x: number; y: number } | null = null;

    // Lasso tool state
    var _lassoPoints: number[] = [];
    var _lassoLine: Konva.Line | null = null;
    var _lassoSelection: number[] | null = null;

    // Clone stamp state
    var _cloneSource: { x: number; y: number } | null = null;
    var _cloneOffset: { x: number; y: number } | null = null;
    var _cloneCrosshair: Konva.Shape | null = null;
    ...
    var _clipboard: string | null = null;

    function isDrawTool(name: string): boolean {
        return name === 'brush' || name === 'eraser' || name === 'mask';
    }

    function getStrokeColor(toolName: string, brushColor: string): string {
        if (toolName === 'mask') return 'rgba(239, 68, 68, 0.5)';
        if (toolName === 'eraser') return '#000';
        return brushColor;
    }

    function getCompositeOp(toolName: string): string {
        return toolName === 'eraser' ? 'destination-out' : 'source-over';
    }

    function scheduleLayerDraw(layer: Konva.Layer): void {
        if (!_drawScheduled) {
            _drawScheduled = true;
    ...
    var BrushTool: CanvasTool = {
        ...
        onMouseDown: function(ctx, e) {
            ...
            if (e.target && e.target.name() && (e.target.name() === 'bounding-box' || e.target.name().indexOf('handle-') === 0)) return;
            _drawing = true;
        ...
        onMouseUp: function(ctx) {
            if (_drawing && _currentLine) ctx.pushHistoryGrouped();
            _drawing = false;
            _currentLine = null;
        }
    };
    ...
    var RectTool: CanvasTool = {
        ...
        onMouseDown: function(ctx, e) {
            var al = ctx.getActiveLayer();
            if (!al || al.data.locked) return;
            if (al.data.type !== 'draw' && al.data.type !== 'mask') return;
        ...
        onMouseMove: function(ctx, pos) {
            if (!_rectStart || !_rectPreview) return;
            ...
            _rectPreview.x(x); _rectPreview.y(y);
            _rectPreview.width(w); _rectPreview.height(h);
    ...
    // ── Gradient Tool ──
    var GradientTool: CanvasTool = {
        name: 'gradient',
        cursor: 'crosshair',
        showsBrushCursor: false,
        onMouseDown: function(ctx, e) {
            var al = ctx.getActiveLayer();
            if (!al || al.data.locked || al.data.type !== 'draw') return;
        ...
        onMouseMove: function(ctx, pos) {
            if (!_gradStart || !_gradPreview) return;
    ...
    var MoveTool: CanvasTool = {
        ...
        onMouseDown: function(ctx, e) {
            var target = e.target;
            if (!target || target === ctx.stage) return;
            if (target.name() === 'bounding-box' || target.name().indexOf('handle-') === 0) return;
        ...
        onMouseMove: function(ctx, pos) {
            if (!_moveTarget || !_moveStart || !_moveNodeStart) return;
            ...
            if (parent) parent.batchDraw();
        },
        onMouseUp: function(ctx) {
            if (_moveTarget) ctx.pushHistory();
            _moveTarget = null;
            _moveStart = null;
            _moveNodeStart = null;
        },
        onKeyDown: function(ctx, e) {
            ...
            al.konvaLayer.getChildren().forEach(function(child) {
                switch (e.code) {
                    case 'ArrowUp': child.y(child.y() - step); moved = true; break;
                    case 'ArrowDown': child.y(child.y() + step); moved = true; break;
                    case 'ArrowLeft': child.x(child.x() - step); moved = true; break;
                    case 'ArrowRight': child.x(child.x() + step); moved = true; break;
                }
            });
            if (moved) {
                e.preventDefault();
    ...
    // ── Color Picker Tool ──
    var ColorPickerTool: CanvasTool = {
        name: 'colorpicker',
        cursor: 'crosshair',
        showsBrushCursor: false,
    ...
    var TextTool: CanvasTool = {
        ...
        onMouseDown: function(ctx, e) {
            ...
            if (al && al.data.type === 'text') {
                var kText = al.konvaLayer.findOne('Text') as Konva.Text | undefined;
                if (kText) {
                    kText.x(pos.x);
                    kText.y(pos.y);
                    al.konvaLayer.batchDraw();
            ...
            var td = newLayer.data as TextLayerData;
            td.position = { x: pos.x, y: pos.y };
            var kTextNew = newLayer.konvaLayer.findOne('Text') as Konva.Text | undefined;
            if (kTextNew) {
                kTextNew.x(pos.x);
                kTextNew.y(pos.y);
    ...
    function floodFill(imageData: ImageData, startX: number, startY: number, fillColor: number[], threshold: number): void {
        ...
        if (targetR === fillColor[0] && targetG === fillColor[1] && targetB === fillColor[2] && Math.abs(targetA - fillColor[3]) < 5) return;
        ...
        while (stack.length > 0) {
            ...
            if (dr + dg + db + da > threshold * 4) continue;
    ...
    var LassoTool: CanvasTool = {
        ...
        onMouseDown: function(ctx, _e) {
            ...
            if (_lassoLine) { _lassoLine.destroy(); _lassoLine = null; }
    ...
    var CloneStampTool: CanvasTool = {
        ...
        onMouseDown: function(ctx, e) {
            ...
            if (e.evt.altKey) {
                ...
                if (_cloneCrosshair) _cloneCrosshair.destroy();
        ...
        onMouseMove: function(ctx, pos) {
            if (!_drawing || !_cloneSource || !_cloneOffset) return;
        ...
        onMouseUp: function(ctx) {
            if (_drawing) ctx.pushHistoryGrouped();
            _drawing = false;
        },
        onDeactivate: function(ctx) {
            if (_cloneCrosshair) { _cloneCrosshair.destroy(); _cloneCrosshair = null; }
            _cloneSource = null;
            _cloneOffset = null;
            ctx.uiLayer.batchDraw();
        }
    ...
    var _tools: Record<string, CanvasTool> = {
        select:      BboxTool,
        brush:       BrushTool,
        eraser:      EraserTool,
        mask:        MaskTool,
    ...
    function registerTool(name: string, tool: CanvasTool): void {
        _tools[name] = tool;
    }

    function get(name: string): CanvasTool | null {
        return _tools[name] || null;
    }

    function all(): Record<string, CanvasTool> {
        return _tools;
    }
    ...
    function getLassoSelection(): number[] | null {
        return _lassoSelection;
    }

    function clearLassoSelection(ctx: CanvasToolContext): void {
        if (_lassoLine) { _lassoLine.destroy(); _lassoLine = null; }
        _lassoSelection = null;
        _lassoPoints = [];
        ctx.uiLayer.batchDraw();
    ...
    function convertLassoToMask(ctx: CanvasToolContext): void {
        if (!_lassoSelection || _lassoSelection.length < 6) return;
    ...
    function getFillThreshold(): number { return _fillThreshold; }
    function setFillThreshold(t: number): void { _fillThreshold = Math.max(0, Math.min(255, t)); }

    function getBrushOpacity(): number { return _brushOpacity; }
    function setBrushOpacity(o: number): void { _brushOpacity = Math.max(0, Math.min(1, o)); }

    function getClipboard(): string | null { return _clipboard; }
    function setClipboard(data: string): void { _clipboard = data; }
