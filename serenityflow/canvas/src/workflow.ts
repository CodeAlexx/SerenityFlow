...
/** A ComfyUI litegraph node from a graph export */
...
/** ComfyUI graph export format (nodes[] + links[]) */
interface ComfyGraphData {
    nodes: ComfyGraphNode[];
    links?: Array<unknown[] | Record<string, unknown>>;
    definitions?: { subgraphs?: Array<{ id: string; nodes?: ComfyGraphNode[]; links?: Array<unknown[] | Record<string, unknown>> }> };
}

function serializeWorkflow(canvas: SFCanvas) {
    const prompt: ComfyPrompt = {};
    const nodePositions: Record<string, { type: string; pos: [number, number]; size: [number, number]; widgets_values: Record<string, unknown>; collapsed: boolean }> = {};

    canvas.nodes.forEach((node: SFNode, id: string) => {
        const inputs: Record<string, unknown> = {};

        node.inputs.forEach((input: SFNodeInput, idx: number) => {
            const conn = canvas.connections.find(
                (c: SFConnection) => c.targetNode === id && c.targetSlot === idx
            );
            if (conn) {
 ...
 */
function loadWorkflow(canvas: SFCanvas, data: Record<string, unknown>, nodeInfo: ComfyObjectInfo): void {
    try {
        if (data.version && data.prompt) {
            ...
            deserializeWorkflow(canvas, data.prompt as ComfyPrompt, (data.nodes as Record<string, { pos?: [number, number]; widgets_values?: Record<string, unknown>; collapsed?: boolean }>) || null, nodeInfo);
        } else if (data.nodes && Array.isArray(data.nodes)) {
            console.log('[workflow] Loading ComfyUI graph format,', data.nodes.length, 'top-level nodes');
            loadComfyUIGraph(canvas, data as unknown as ComfyGraphData, nodeInfo);
        } else if (typeof data === 'object' && !Array.isArray(data)) {
            const firstVal = Object.values(data)[0] as Record<string, unknown> | undefined;
            if (firstVal && firstVal.class_type) {
                console.log('[workflow] Loading raw API format');
                deserializeWorkflow(canvas, data as unknown as ComfyPrompt, null, nodeInfo);
            } else {
 ...
 */
function loadComfyUIGraph(canvas: SFCanvas, graphData: ComfyGraphData, nodeInfo: ComfyObjectInfo) {
    // Clear
    ...
    const subgraphDefs: Record<string, { nodes?: ComfyGraphNode[]; links?: Array<unknown[] | Record<string, unknown>> }> = {};
    ...
    const linkMap: Record<string, { srcId: string; srcSlot: number; dstId: string; dstSlot: number; type: string }> = {};
    for (const link of allLinks) {
        ...
        linkMap[String(id)] = { srcId: String(srcId), srcSlot: srcSlot as number, dstId: String(dstId), dstSlot: dstSlot as number, type: String(ltype) };
    }

    // Build a map of graph node inputs: graphNodeId -> [{name, link_id, slot}]
    const graphNodeInputs: Record<string, Array<{ name: string; link: unknown; slot: number }>> = {};
    for (const node of allNodes) {
        const nid = String(node.id);
        const inputs = node.inputs || [];
        graphNodeInputs[nid] = inputs.map((inp, idx: number) => ({
    ...
    // Create canvas nodes
    const idMap: Record<string, string> = {}; // graph node id -> canvas node id
    let created = 0;
    ...
    for (const node of allNodes) {
        ...
        const info: ComfyNodeInfo = nodeInfo ? (nodeInfo[ntype] || {}) : {};
        ...
        if (Array.isArray(pos) && pos.length >= 2) {
            ...
            px = (pos as Record<string, number>)['0'] || 0; py = (pos as Record<string, number>)['1'] || 0;
        }
 ...
 */
function _applyWidgetValues(canvasNode: SFNode, widgetValues: unknown[], info: ComfyNodeInfo) {
 ...
 */
function deserializeWorkflow(canvas: SFCanvas, prompt: ComfyPrompt, nodePositions: Record<string, { pos?: [number, number]; widgets_values?: Record<string, unknown>; collapsed?: boolean }> | null, nodeInfo: ComfyObjectInfo): void {
    ...
    for (const [origId, nodeData] of Object.entries(prompt) as [string, ComfyPromptNode][]) {
        if (!nodeData || !nodeData.class_type) continue;

        const info: ComfyNodeInfo = nodeInfo ? (nodeInfo[nodeData.class_type] || {}) : {};
 ...
 */
function _updateModelBadgeFromWorkflow(nodes: Array<{ type?: string; widgets_values?: unknown[] }>) {
 ...
 */
function _updateModelBadgeFromPrompt(prompt: ComfyPrompt) {
    var loaderTypes = ['CheckpointLoaderSimple', 'UNETLoader', 'LTXVLoader'];
 ...
 */
function _setModelBadge(modelName: string) {
