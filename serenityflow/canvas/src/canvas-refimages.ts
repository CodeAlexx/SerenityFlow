...
interface RefImage {
    ...
    src: string;          // data URL
    weight: number;       // 0..2
    method: string;       // style | face | composition | full
    model: string;        // ip-adapter model name
...
var CanvasRefImages = (function() {
    ...
    function add(src: string): RefImage {
        var ref: RefImage = {
            id: 'ref_' + (++_nextId),
            src: src,
            weight: 1.0,
    ...
    function getAll(): RefImage[] { return _refs; }

    function getForPayload(): Array<{ imageId: string; weight: number; method: string; model: string }> {
    ...
    function bindPanelEvents(): void {
        ...
        var fileInput = document.getElementById('ref-file-input') as HTMLInputElement | null;
        if (addBtn && fileInput) {
            ...
            fileInput.addEventListener('change', function() {
                ...
                Array.from(fileInput!.files).forEach(function(file) {
                    ...
                    reader.onload = function(ev) { add((ev.target as FileReader).result as string); };
        ...
        document.querySelectorAll('.ref-weight').forEach(function(el) {
            ...
            input.addEventListener('input', function() {
                ...
                var ref = _refs.find(function(r: RefImage) { return r.id === targetId; });
                if (ref) {
                    ref.weight = parseFloat(input.value);
                    var valEl = input.parentElement!.querySelector('.ref-weight-val');
                    if (valEl) valEl.textContent = ref.weight.toFixed(2);
                }
            });
        });
        ...
        document.querySelectorAll('.ref-method').forEach(function(el) {
            ...
            select.addEventListener('change', function() {
                ...
                var ref = _refs.find(function(r: RefImage) { return r.id === targetId; });
                if (ref) ref.method = select.value;
            });
        });
...
var CanvasContextMenu = (function() {
    ...
    var _menuEl: HTMLElement | null = null;
    var _ctx: CanvasToolContext | null = null;

    function show(x: number, y: number, ctx: CanvasToolContext): void {
        _ctx = ctx;
        if (!_menuEl) {
        ...
        var items: Array<{ label: string; action: string; divider?: boolean }> = [];
        ...
        items.forEach(function(item) {
            if (item.divider) html += '<div class="ctx-divider"></div>';
    ...
    function _outsideClick(): void { hide(); }
    ...
    function handleAction(action: string): void {
        ...
        switch (action) {
            case 'duplicate': _ctx.duplicateActiveLayer(); break;
            case 'delete': _ctx.deleteActiveLayer(); break;
            case 'merge':
                if (al) {
                ...
                break;
            case 'flatten': _ctx.flattenVisible(); break;
            case 'togglelock':
                if (al) { al.data.locked = !al.data.locked; }
                break;
            case 'invert_mask': invertMask(_ctx); break;
            case 'expand_mask': morphMask(_ctx, 'dilate', 5); break;
            case 'shrink_mask': morphMask(_ctx, 'erode', 5); break;
            case 'feather_mask': morphMask(_ctx, 'blur', 3); break;
            case 'paste':
    ...
    function invertMask(ctx: CanvasToolContext): void {
        var al = ctx.getActiveLayer();
        if (!al || al.data.type !== 'mask') return;
    ...
    function morphMask(ctx: CanvasToolContext, op: string, radius: number): void {
        var al = ctx.getActiveLayer();
        if (!al || al.data.type !== 'mask') return;
        var bb = ctx.boundingBox;
        var bw = Math.round(bb.width()), bh = Math.round(bb.height());
        ...
        img.onload = function() {
            ...
            if (op === 'blur') {
                ...
                var imageData = gc.getImageData(0, 0, bw, bh);
                var d = imageData.data;
                for (var i = 0; i < d.length; i += 4) d[i + 3] = 255 - d[i + 3];
                gc.putImageData(imageData, 0, 0);
                ...
                d = imageData.data;
                for (var j = 0; j < d.length; j += 4) d[j + 3] = 255 - d[j + 3];
                gc.putImageData(imageData, 0, 0);
            }
...
var SmartCrop = (function() {
    ...
    function checkBboxCutsObjects(ctx: CanvasToolContext): { cuts: boolean; suggestion?: { x: number; y: number; w: number; h: number } } {
        // Check if SAM instances exist and bbox cuts through any
        if (typeof CanvasSAM === 'undefined') return { cuts: false };
        var instances = CanvasSAM.getInstances();
        if (instances.length === 0) return { cuts: false };
    ...
    function snapToObjects(ctx: CanvasToolContext): void {
        var result = checkBboxCutsObjects(ctx);
        if (!result.cuts || !result.suggestion) return;
...
var CanvasZoom = (function() {
    ...
    var _displayEl: HTMLElement | null = null;

    function fitToScreen(ctx: CanvasToolContext): void {
        var bb = ctx.boundingBox;
    ...
    function zoomTo100(ctx: CanvasToolContext): void {
        var bb = ctx.boundingBox;
        var sw = ctx.stage.width();
        var sh = ctx.stage.height();
    ...
    function getZoomPercent(ctx?: CanvasToolContext | null): number {
        if (!ctx || !ctx.stage) return 100;
        return Math.round(ctx.stage.scaleX() * 100);
    }

    function updateDisplay(ctx: CanvasToolContext): void {
        if (!_displayEl) {
...
var LayerThumbnails = (function() {
    ...
    function generateThumbnail(cl: CanvasLayer): string {
        try {
    ...
    function getSize(): number { return THUMB_SIZE; }
...
var LayerSolo = (function() {
    ...
    var _soloLayerId: number | null = null;
    var _savedVisibility: Map<number, boolean> = new Map();

    function toggle(layerId: number, canvasLayers: CanvasLayer[]): void {
    ...
    function isSoloed(layerId: number): boolean {
        return _soloActive && _soloLayerId === layerId;
    }

    function isActive(): boolean { return _soloActive; }
