...
var CanvasStaging = (function() {
    ...
    var _stagingLayer: Konva.Layer | null = null;
    var _stagingImage: Konva.Image | null = null;
    var _stagingBorder: Konva.Rect | null = null;
    var _stagingLabel: Konva.Text | null = null;
    var _partialMaskLayer: Konva.Layer | null = null;
    var _panelEl: HTMLElement | null = null;

    // Reference to original canvas state for compare mode
    var _originalSnapshot: string | null = null;
    var _originalImage: Konva.Image | null = null;
    var _showingOriginal = false;

    // Partial mask drawing state
    var _partialDrawing = false;
    var _partialLine: Konva.Line | null = null;

    // ── Context reference (set when staging activates) ──
    var _ctx: CanvasToolContext | null = null;

    // ── Core ──

    function activate(results: StagedResult[], ctx: CanvasToolContext): void {
        if (_state.active) deactivate();
    ...
    function deactivate(): void {
        ...
        if (_stagingImage) { _stagingImage.destroy(); _stagingImage = null; }
        if (_stagingBorder) { _stagingBorder.destroy(); _stagingBorder = null; }
        if (_stagingLabel) { _stagingLabel.destroy(); _stagingLabel = null; }
        if (_originalImage) { _originalImage.destroy(); _originalImage = null; }
        if (_stagingLayer) { _stagingLayer.destroyChildren(); _stagingLayer.batchDraw(); }
        if (_partialMaskLayer) { _partialMaskLayer.destroyChildren(); _partialMaskLayer.batchDraw(); }
    ...
    function showResult(index: number, ctx: CanvasToolContext): void {
        ...
        img.onload = function() {
            ...
            if (_stagingBorder) _stagingBorder.moveToTop();
            if (_stagingLabel) _stagingLabel.moveToTop();
            _stagingLayer!.batchDraw();
        };
        img.src = result.src;
    ...
    function updateLabel(): void {
        if (!_stagingLabel) return;
        var total = _state.results.length;
        var current = _state.currentIndex + 1;
        var text = 'Result ' + current + ' of ' + total;
        if (_showingOriginal) text = 'Original (compare)';
        if (_state.partialMaskMode) text += ' — Paint to accept';
        _stagingLabel.text(text);
        if (_stagingLayer) _stagingLayer.batchDraw();
    }

    // ── Navigation ──

    function nextResult(): void {
        if (!_state.active || !_ctx) return;
    ...
    function acceptFull(ctx: CanvasToolContext, result: StagedResult): void {
        if (result.isVideo) {
    ...
    function acceptPartial(ctx: CanvasToolContext, result: StagedResult): void {
        ...
        resultImg.onload = function() { loaded++; if (loaded === 2) onBothLoaded(); };
        maskImg.onload = function() { loaded++; if (loaded === 2) onBothLoaded(); };
    ...
    function captureOriginal(ctx: CanvasToolContext): void {
        var bb = ctx.boundingBox;
        try {
            ctx.uiLayer.hide();
    ...
    function toggleCompare(): void {
        if (!_state.active || !_ctx || !_originalSnapshot) return;
        _state.compareMode = !_state.compareMode;
        _showingOriginal = _state.compareMode;

        if (_showingOriginal) {
            // Show original
            if (_stagingImage) _stagingImage.visible(false);
            if (!_originalImage) {
            ...
            // Show result
            if (_originalImage) _originalImage.visible(false);
            if (_stagingImage) _stagingImage.visible(true);
            _stagingLayer!.batchDraw();
        }
    ...
    function togglePartialMask(): void {
    ...
    function handlePartialMouseDown(ctx: CanvasToolContext, pos: { x: number; y: number }): void {
        if (!_state.partialMaskMode || !_partialMaskLayer) return;
    ...
    function handlePartialMouseMove(pos: { x: number; y: number }): void {
        if (!_partialDrawing || !_partialLine) return;
        _partialLine.points(_partialLine.points().concat([pos.x, pos.y]));
        if (_partialMaskLayer) _partialMaskLayer.batchDraw();
    }

    function handlePartialMouseUp(): void {
    ...
    function updatePanel(): void {
        ...
        if (partialBtn) partialBtn.addEventListener('click', togglePartialMask);
    }

    // ── Public API ──

    function isActive(): boolean { return _state.active; }
    function getCurrentIndex(): number { return _state.currentIndex; }
    function getResults(): StagedResult[] { return _state.results; }
    function isCompareMode(): boolean { return _state.compareMode; }
    function isPartialMaskMode(): boolean { return _state.partialMaskMode; }
