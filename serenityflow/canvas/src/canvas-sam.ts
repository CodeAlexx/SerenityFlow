...
interface SamInstance {
    instance_id: string;
    mask_png: string;         // base64 PNG (grayscale mask)
...
var CanvasSAM = (function() {
    ...
    var _mode: 'text' | 'click' | 'exemplar' = 'text';
    var _instances: SamInstance[] = [];
    var _overlayImages: Array<{ img: Konva.Image; id: string }> = [];
    var _clickPoints: Array<{ x: number; y: number; label: number }> = [];
    var _clickMarkers: Konva.Circle[] = [];
    var _exemplarRect: Konva.Rect | null = null;
    var _exemplarStart: { x: number; y: number } | null = null;
    var _loading = false;
    var _debounceTimer: ReturnType<typeof setTimeout> | null = null;
    var _resultsPanel: HTMLElement | null = null;
    ...
    function captureCanvasComposite(ctx: CanvasToolContext): Promise<Blob> {
    ...
    function sendTextRequest(ctx: CanvasToolContext, prompt: string, threshold: number): void {
        if (_loading) return;
        _loading = true;
        updateLoadingState();
    ...
    function sendPointsRequest(ctx: CanvasToolContext): void {
        if (_loading || _clickPoints.length === 0) return;
        _loading = true;
        updateLoadingState();
    ...
    function sendExemplarRequest(ctx: CanvasToolContext, bbox: { x: number; y: number; width: number; height: number }): void {
        if (_loading) return;
        _loading = true;
        updateLoadingState();
    ...
    function handleResults(ctx: CanvasToolContext, instances: SamInstance[]): void {
        clearOverlays(ctx);
    ...
    function clearOverlays(ctx: CanvasToolContext): void {
        _overlayImages.forEach(function(o) { o.img.destroy(); });
        _overlayImages = [];
        _clickMarkers.forEach(function(m) { m.destroy(); });
        _clickMarkers = [];
        if (_exemplarRect) { _exemplarRect.destroy(); _exemplarRect = null; }
        ctx.uiLayer.batchDraw();
    }

    function toggleInstance(ctx: CanvasToolContext, instanceId: string, selected: boolean): void {
        _instances.forEach(function(inst) {
            if (inst.instance_id === instanceId) inst.selected = selected;
        });
        // Show/hide overlay
    ...
    function applySelected(ctx: CanvasToolContext): void {
    ...
    function applyAll(ctx: CanvasToolContext): void {
        _instances.forEach(function(i) { i.selected = true; });
        applySelected(ctx);
    }

    function cancelResults(ctx: CanvasToolContext): void {
        clearOverlays(ctx);
        hideResultsPanel();
        _instances = [];
        _clickPoints = [];
    ...
    function showResultsPanel(ctx: CanvasToolContext): void {
        if (!_resultsPanel) {
    ...
    function hideResultsPanel(): void {
        if (_resultsPanel) _resultsPanel.style.display = 'none';
    }

    function renderResultsList(ctx: CanvasToolContext): void {
        ...
        _resultsPanel.querySelectorAll('.sam-result-check').forEach(function(cb) {
            (cb as HTMLInputElement).addEventListener('change', function(this: HTMLInputElement) {
    ...
    function showSamError(msg: string): void {
        updateLoadingState();
    ...
    var SelectObjectTool: CanvasTool = {
        name: 'sam',
        cursor: 'crosshair',
    ...
    function showSamToolbar(): void {
        ...
        toolbar.querySelectorAll('.sam-mode-btn').forEach(function(btn) {
            (btn as HTMLElement).addEventListener('click', function(this: HTMLElement) {
                _mode = this.dataset.mode as any;
                toolbar.querySelectorAll('.sam-mode-btn').forEach(function(b) {
                    (b as HTMLElement).classList.remove('active');
                });
                this.classList.add('active');
                var textInput = document.getElementById('sam-text-input') as HTMLElement;
                var detectBtn = document.getElementById('sam-detect-btn') as HTMLElement;
                if (textInput) textInput.style.display = _mode === 'text' ? '' : 'none';
                if (detectBtn) detectBtn.style.display = _mode === 'text' ? '' : 'none';
            });
        });

        // Detect button and Enter key
        var detectBtn = document.getElementById('sam-detect-btn');
        var textInput = document.getElementById('sam-text-input') as HTMLInputElement;
        if (detectBtn) {
        ...
        toolbar.querySelectorAll('.sam-preset-btn').forEach(function(btn) {
            (btn as HTMLElement).addEventListener('click', function(this: HTMLElement) {
                ...
                _mode = 'text';
                toolbar.querySelectorAll('.sam-mode-btn').forEach(function(b) {
                    (b as HTMLElement).classList.toggle('active', (b as HTMLElement).dataset.mode === 'text');
                });
    ...
    function triggerTextDetect(prompt?: string): void {
        var textInput = document.getElementById('sam-text-input') as HTMLInputElement | null;
        var text = prompt || (textInput ? textInput.value.trim() : '');
        if (!text) return;
        // We need the tool context — get it from CanvasTab
        var ctx = (window as any)._samToolContext;
        if (ctx) {
            sendTextRequest(ctx, text, 0.3);
        }
    }

    function hideSamToolbar(): void {
    ...
    function getMode(): string { return _mode; }
    function setMode(m: 'text' | 'click' | 'exemplar'): void { _mode = m; }
    function isLoading(): boolean { return _loading; }
    function getInstances(): SamInstance[] { return _instances; }
    function getTool(): CanvasTool { return SelectObjectTool; }
