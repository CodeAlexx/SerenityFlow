...
class SFSidebar {
    canvas: SFCanvas;
    api: SFApiInstance;
    nodeTypes: ComfyObjectInfo;
    categories: Record<string, string[]>; // category -> [class_type]
    searchInput: HTMLInputElement;
    nodeList: HTMLElement;

    constructor(canvas: SFCanvas, api: SFApiInstance) {
        this.canvas = canvas;
        this.api = api;
        this.nodeTypes = {};  // class_type -> info
        this.categories = {}; // category -> [class_type]

        this.searchInput = document.getElementById('node-search') as HTMLInputElement;
    ...
    _addNodeAtCenter(nodeType: string): void {
    ...
    addNodeAtPosition(nodeType: string, screenX: number, screenY: number): SFNode | null {
        const info = this.nodeTypes[nodeType];
        if (!info) return null;
