...
var Settings = (function() {
    ...
    function merge(defaults: Record<string, any>, overrides: Record<string, any>): Record<string, any> {
    ...
    function get(key: string) { return current[key]; }
    ...
    function applyImmediate(key: string, value: unknown) {
    ...
    function applyTheme(themeId: string) {
        ...
        var t = (THEMES as Record<string, any>)[themeId] || THEMES.dark;
...
var SettingsTab = (function() {
    ...
    function bindNav() {
        ...
        nav.addEventListener('click', function(e) {
            var item = (e.target as HTMLElement).closest('.settings-nav-item') as HTMLElement | null;
            if (!item) return;
            activeSection = item.dataset.section!;
            nav!.querySelectorAll('.settings-nav-item').forEach(function(n) {
                (n as HTMLElement).classList.toggle('active', (n as HTMLElement).dataset.section === activeSection);
            });
    ...
    function bindMemoryEvents() {
        var vramSlider = document.getElementById('settings-vram-slider') as HTMLInputElement | null;
        var vramValue = document.getElementById('settings-vram-value');
        if (vramSlider) {
            vramSlider.addEventListener('input', function(this: HTMLInputElement) {
                var v = parseInt(this.value);
                vramValue!.textContent = v === 0 ? 'Auto' : formatMb(v);
                Settings.set('vramBudgetMb', v);
            });
        }
        var pinnedSlider = document.getElementById('settings-pinned-slider') as HTMLInputElement | null;
        var pinnedValue = document.getElementById('settings-pinned-value');
        if (pinnedSlider) {
            pinnedSlider.addEventListener('input', function(this: HTMLInputElement) {
                var v = parseInt(this.value);
        ...
        fetch('/system_stats').then(function(r) { return r.json(); }).then(function(info) {
            ...
            if (gpuLabel) {
                if (info.devices && info.devices.length > 0) {
                    ...
                    var slider = document.getElementById('settings-vram-slider') as HTMLInputElement | null;
    ...
    function updateMemoryBars(vramUsed: number, vramTotal: number, ramUsed: number, ramTotal: number) {
    ...
    function bindModelsEvents() {
        ...
        var addInput = document.getElementById('settings-add-model-path') as HTMLInputElement | null;
        if (addBtn && addInput) {
    ...
    function loadModelPaths() {
        fetch('/folder_paths')
            ...
            .then(function(data) {
                ...
                var allPaths = new Set<string>();
                Object.keys(data).forEach(function(cat) {
                    if (data[cat].paths) {
                        data[cat].paths.forEach(function(p: string) { allPaths.add(p); });
                    }
                });
                // Group by parent dir
                var parents: Record<string, string[]> = {};
                allPaths.forEach(function(p: string) {
    ...
    function bindOutputEvents() {
        ...
        var jpegSlider = document.getElementById('settings-jpeg-quality') as HTMLInputElement | null;
        var jpegVal = document.getElementById('settings-jpeg-val');
        if (jpegSlider) {
            jpegSlider.addEventListener('input', function(this: HTMLInputElement) {
        ...
        if (pillRow) {
            pillRow.addEventListener('click', function(e) {
                var pill = (e.target as HTMLElement).closest('.settings-pill') as HTMLElement | null;
                if (!pill) return;
                var fmt = pill.dataset.format;
                Settings.set('outputFormat', fmt);
                pillRow!.querySelectorAll('.settings-pill').forEach(function(p) {
                    (p as HTMLElement).classList.toggle('active', (p as HTMLElement).dataset.format === fmt);
                });
    ...
    function loadOutputFiles() {
        fetch('/output_files').then(function(r) { return r.json(); }).then(function(files) {
            ...
            list.innerHTML = '';
            files.forEach(function(f: { name: string; size_bytes: number; modified: number }) {
            ...
            list!.onclick = function(e) {
                var del = (e.target as HTMLElement).closest('.settings-file-delete') as HTMLElement | null;
                if (!del) return;
    ...
    function buildInterfaceSection() {
        ...
        Object.keys(THEMES).forEach(function(id) {
            var t = (THEMES as Record<string, any>)[id];
        ...
        document.querySelectorAll('.settings-theme-card').forEach(function(card) {
            card.addEventListener('click', function(this: HTMLElement) {
                ...
                document.querySelectorAll('.settings-theme-card').forEach(function(c) {
                    (c as HTMLElement).classList.toggle('active', (c as HTMLElement).dataset.theme === Settings.get('theme'));
                });
            });
        });
        // Accent swatches
        document.querySelectorAll('.settings-swatch').forEach(function(sw) {
            sw.addEventListener('click', function(this: HTMLElement) {
                ...
                document.querySelectorAll('.settings-swatch').forEach(function(s) {
                    (s as HTMLElement).classList.toggle('active', (s as HTMLElement).dataset.color === Settings.get('accentColor'));
                });
                var customInput = document.getElementById('settings-custom-accent') as HTMLInputElement | null;
                if (customInput) customInput.value = Settings.get('accentColor');
            });
        });
        // Custom accent
        var customAccent = document.getElementById('settings-custom-accent') as HTMLInputElement | null;
        if (customAccent) {
            customAccent.addEventListener('blur', function(this: HTMLInputElement) {
        ...
        document.querySelectorAll('[data-mode]').forEach(function(pill) {
            pill.addEventListener('click', function(this: HTMLElement) {
                ...
                document.querySelectorAll('[data-mode]').forEach(function(p) {
                    (p as HTMLElement).classList.toggle('active', (p as HTMLElement).dataset.mode === Settings.get('defaultMode'));
                });
            });
        });
        // Column pills
        document.querySelectorAll('[data-cols]').forEach(function(pill) {
            pill.addEventListener('click', function(this: HTMLElement) {
                ...
                document.querySelectorAll('[data-cols]').forEach(function(p) {
                    (p as HTMLElement).classList.toggle('active', parseInt((p as HTMLElement).dataset.cols!) === Settings.get('galleryColumns'));
                });
            });
        });
    ...
    // ── Helpers ──
    function bindInput(id: string, key: string, type?: string) {
        var el = document.getElementById(id) as HTMLInputElement | null;
        if (!el) return;
        el.addEventListener('blur', function(this: HTMLInputElement) {
            var v: string | number = this.value;
            if (type === 'int') v = parseInt(v) || (Settings.DEFAULTS as Record<string, unknown>)[key] as number;
            else if (type === 'float') v = parseFloat(v) || (Settings.DEFAULTS as Record<string, unknown>)[key] as number;
            Settings.set(key, v);
        });
    }

    function bindSelect(id: string, key: string) {
        var el = document.getElementById(id) as HTMLSelectElement | null;
        if (!el) return;
        el.addEventListener('change', function(this: HTMLSelectElement) { Settings.set(key, this.value); });
    }
    ...
    function formatMb(mb: number) { return (mb / 1024).toFixed(1) + ' GB'; }
