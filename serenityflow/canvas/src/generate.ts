...
var GenerateTab = (function() {
    ...
    interface GenerateState {
        ...
        autoSwitchNew: boolean;
        galleryPage: number;
        galleryPageSize: number;
        contextMenuIndex: number;
        gallerySettingsOpen: boolean;
        ...
        seamlessY: boolean;
        refinerModel: string;
        refinerScheduler: string;
        refinerSteps: number;
        refinerCfg: number;
        refinerStart: number;
        aestheticScore: number;
        negAestheticScore: number;
    ...
    interface GenerateEls {
        ...
        imageHeader: HTMLElement | null;
        imageBody: HTMLElement | null;
        settingsHeader: HTMLElement | null;
        settingsBody: HTMLElement | null;
        ...
        scheduler: HTMLSelectElement | null;
        seed: HTMLInputElement | null;
        seedShuffle: HTMLElement | null;
        btn: HTMLButtonElement | null;
        empty: HTMLElement | null;
        previewImg: (HTMLImageElement & { _previewUrl?: string }) | null;
        ...
        leftProgress: HTMLElement | null;
        leftProgressBar: HTMLElement | null;
        leftProgressLabel: HTMLElement | null;
        errorBanner: HTMLElement | null;
        wsIndicator: HTMLElement | null;
        galleryGrid: HTMLElement | null;
        galleryClear: HTMLElement | null;
        leftPanel: HTMLElement | null;
        rightPanel: HTMLElement | null;
        aspectDropdown: HTMLSelectElement | null;
        aspectPreview: HTMLElement | null;
        modelBadge: HTMLElement | null;
        toolbarBatchInput: HTMLInputElement | null;
    }
    ...
    var els: GenerateEls = {} as GenerateEls;
    ...
    function cacheElements() {
        els.model = document.getElementById('gen-model') as HTMLInputElement | null;
        els.modelSearch = document.getElementById('gen-model-search') as HTMLInputElement | null;
        ...
        els.prompt = document.getElementById('gen-prompt') as HTMLTextAreaElement | null;
        els.negPrompt = document.getElementById('gen-neg-prompt') as HTMLTextAreaElement | null;
        els.negSection = document.getElementById('gen-neg-section');
        els.customWidth = document.getElementById('gen-custom-width') as HTMLInputElement | null;
        els.customHeight = document.getElementById('gen-custom-height') as HTMLInputElement | null;
        els.videoSection = document.getElementById('gen-video-section');
        els.secondsInput = document.getElementById('gen-seconds') as HTMLInputElement | null;
        els.fpsInput = document.getElementById('gen-fps') as HTMLInputElement | null;
        els.fpsRange = document.getElementById('gen-fps-range') as HTMLInputElement | null;
        ...
        els.steps = document.getElementById('gen-steps') as HTMLInputElement | null;
        els.stepsRange = document.getElementById('gen-steps-range') as HTMLInputElement | null;
        els.cfgRow = document.getElementById('gen-cfg-row');
        els.cfg = document.getElementById('gen-cfg') as HTMLInputElement | null;
        els.cfgRange = document.getElementById('gen-cfg-range') as HTMLInputElement | null;
        els.guidanceRow = document.getElementById('gen-guidance-row');
        els.guidance = document.getElementById('gen-guidance') as HTMLInputElement | null;
        els.guidanceRange = document.getElementById('gen-guidance-range') as HTMLInputElement | null;
        els.scheduler = document.getElementById('gen-scheduler') as HTMLSelectElement | null;
        els.seed = document.getElementById('gen-seed') as HTMLInputElement | null;
        els.seedShuffle = document.getElementById('gen-seed-shuffle');
        els.btn = document.getElementById('gen-btn') as HTMLButtonElement | null;
        els.empty = document.getElementById('gen-empty');
        els.previewImg = document.getElementById('gen-preview-img') as (HTMLImageElement & { _previewUrl?: string }) | null;
        els.previewVideo = document.getElementById('gen-preview-video') as HTMLVideoElement | null;
        ...
        els.aspectDropdown = document.getElementById('gen-aspect-dropdown') as HTMLSelectElement | null;
        els.aspectPreview = document.getElementById('gen-aspect-preview');
        els.modelBadge = document.getElementById('gen-model-badge');
        els.toolbarBatchInput = document.getElementById('gen-toolbar-batch-input') as HTMLInputElement | null;
    }
    ...
    function bindEvents() {
        ...
        var coherenceEl = document.getElementById('gen-coherence-mode') as HTMLSelectElement | null;
        if (coherenceEl) coherenceEl.addEventListener('change', function(this: HTMLSelectElement) { state.coherenceMode = this.value; });
        var infillEl = document.getElementById('gen-infill-method') as HTMLSelectElement | null;
        if (infillEl) infillEl.addEventListener('change', function(this: HTMLSelectElement) { state.infillMethod = this.value; });
        ...
        var refSchedEl = document.getElementById('gen-refiner-scheduler') as HTMLSelectElement | null;
        if (refSchedEl) refSchedEl.addEventListener('change', function(this: HTMLSelectElement) { state.refinerScheduler = this.value; });
        var refModelEl = document.getElementById('gen-refiner-model') as HTMLSelectElement | null;
        if (refModelEl) refModelEl.addEventListener('change', function(this: HTMLSelectElement) { state.refinerModel = this.value; });

        // ── Phase 3: Advanced controls ──
        var vaeEl = document.getElementById('gen-vae-picker') as HTMLSelectElement | null;
        if (vaeEl) vaeEl.addEventListener('change', function(this: HTMLSelectElement) { state.vae = this.value; });
        ...
        // Scheduler
        els.scheduler!.addEventListener('change', function(this: HTMLSelectElement) {
        ...
        if (els.aspectDropdown) {
            els.aspectDropdown.addEventListener('change', function(this: HTMLSelectElement) {
                var val = this.value;
        ...
        els.customWidth!.addEventListener('blur', function(this: HTMLInputElement) {
            ...
            this.value = String(v);
            state.width = v;
            var wsl = document.getElementById('gen-width-slider') as HTMLInputElement | null;
            if (wsl) wsl.value = String(v);
            if (state.aspectLocked && state.lockedRatio) {
                ...
                state.height = newH;
                if (els.customHeight) els.customHeight.value = String(newH);
                var hsl = document.getElementById('gen-height-slider') as HTMLInputElement | null;
                if (hsl) hsl.value = String(newH);
            }
            syncAspectDropdown();
            updateAspectPreview();
        });
        els.customHeight!.addEventListener('blur', function(this: HTMLInputElement) {
            var isVideo = ModelUtils.isVideoModel(state.model);
            var v = isVideo
                ? ModelUtils.clampVideoDimension(parseInt(this.value) || 512)
                : ModelUtils.clampDimension(parseInt(this.value) || 1024);
            ...
            state.height = v;
            var hsl = document.getElementById('gen-height-slider') as HTMLInputElement | null;
            if (hsl) hsl.value = String(v);
            if (state.aspectLocked && state.lockedRatio) {
                var newW = isVideo
                    ? ModelUtils.clampVideoDimension(Math.round(v * state.lockedRatio))
                    : ModelUtils.clampDimension(Math.round(v * state.lockedRatio));
                state.width = newW;
                if (els.customWidth) els.customWidth.value = String(newW);
                var wsl = document.getElementById('gen-width-slider') as HTMLInputElement | null;
                if (wsl) wsl.value = String(newW);
            }
            syncAspectDropdown();
            updateAspectPreview();
        });
        ...
        // Width slider sync
        var widthSlider = document.getElementById('gen-width-slider') as HTMLInputElement | null;
        if (widthSlider) {
            widthSlider.addEventListener('input', function(this: HTMLInputElement) {
                var v = parseInt(this.value);
                state.width = v;
                if (els.customWidth) els.customWidth.value = String(v);
                if (state.aspectLocked && state.lockedRatio) {
                    var isVideo = ModelUtils.isVideoModel(state.model);
                    var newH = isVideo
                        ? ModelUtils.clampVideoDimension(Math.round(v / state.lockedRatio))
                    ...
                    state.height = newH;
                    if (els.customHeight) els.customHeight.value = String(newH);
                    var hs = document.getElementById('gen-height-slider') as HTMLInputElement | null;
                    if (hs) hs.value = String(newH);
                }
                syncAspectDropdown();
                updateAspectPreview();
            });
        }
        var heightSlider = document.getElementById('gen-height-slider') as HTMLInputElement | null;
        if (heightSlider) {
            heightSlider.addEventListener('input', function(this: HTMLInputElement) {
                var v = parseInt(this.value);
                state.height = v;
                if (els.customHeight) els.customHeight.value = String(v);
                if (state.aspectLocked && state.lockedRatio) {
                    var isVideo = ModelUtils.isVideoModel(state.model);
                    var newW = isVideo
                        ? ModelUtils.clampVideoDimension(Math.round(v * state.lockedRatio))
                    ...
                    state.width = newW;
                    if (els.customWidth) els.customWidth.value = String(newW);
                    var ws = document.getElementById('gen-width-slider') as HTMLInputElement | null;
                    if (ws) ws.value = String(newW);
                }
                syncAspectDropdown();
                updateAspectPreview();
            });
        ...
        if (seedPrev) {
            seedPrev.addEventListener('click', function() {
                if (state.lastSeed !== null) {
                    ...
                    if (els.seed) els.seed.value = String(state.lastSeed);
                }
            });
        }
        ...
        if (randomToggle) {
            randomToggle.addEventListener('click', function() {
                ...
                if (isRandom) {
                    state.seed = -1;
                    if (els.seed) els.seed.value = String(-1);
                } else {
                    state.seed = Math.floor(Math.random() * 4294967296);
                    if (els.seed) els.seed.value = String(state.seed);
                }
        ...
        var batchInput = document.getElementById('gen-batch') as HTMLInputElement | null;
        if (batchInput) {
            batchInput.addEventListener('input', function(this: HTMLInputElement) {
        ...
        if (els.toolbarBatchInput) {
            els.toolbarBatchInput.addEventListener('input', function(this: HTMLInputElement) {
                state.batchCount = Math.max(1, Math.min(8, parseInt(this.value) || 1));
                if (batchInput) batchInput.value = String(state.batchCount);
            });
        }
        ...
        var loraPicker = document.getElementById('gen-lora-picker') as HTMLSelectElement | null;
        if (loraPicker) {
            loraPicker.addEventListener('change', function(this: HTMLSelectElement) {
        ...
        els.secondsInput!.addEventListener('input', function(this: HTMLInputElement) {
        ...
        var searchInput = document.getElementById('gen-gallery-search-input') as HTMLInputElement | null;
        ...
        // ── Style Preset ──
        var styleSelect = document.getElementById('gen-style-preset') as HTMLSelectElement | null;
        if (styleSelect) {
            styleSelect.addEventListener('change', function(this: HTMLSelectElement) {
        ...
        if (els.prompt) {
            els.prompt.addEventListener('keydown', function(this: HTMLTextAreaElement, e: KeyboardEvent) {
    ...
    function bindAccordion(headerId: string, bodyId: string) {
    ...
    function bindSliderPair(rangeId: string, inputId: string, onUpdate?: (v: string) => void) {
        var range = document.getElementById(rangeId) as HTMLInputElement | null;
    ...
    function syncDimensionInputs() {
        if (els.customWidth) els.customWidth.value = String(state.width);
        if (els.customHeight) els.customHeight.value = String(state.height);
        var ws = document.getElementById('gen-width-slider') as HTMLInputElement | null;
        var hs = document.getElementById('gen-height-slider') as HTMLInputElement | null;
        if (ws) ws.value = String(state.width);
        if (hs) hs.value = String(state.height);
    }

    function getDefaultsForArch(arch: string) {
        var defaults: Record<string, { w: number; h: number }> = {
            sd15:  { w: 512,  h: 512 },
            sdxl:  { w: 1024, h: 1024 },
            sd3:   { w: 1024, h: 1024 },
            flux:  { w: 1024, h: 1024 },
            ...
            .then(function(info: ComfyObjectInfo) {
                ...
                if (vaePicker && info.VAELoader && info.VAELoader.input && info.VAELoader.input.required && info.VAELoader.input.required.vae_name) {
                    ...
                    if (Array.isArray(vaeList)) {
                        vaeList.forEach(function(name: string) {
                ...
                if (refinerPicker && info.CheckpointLoaderSimple && info.CheckpointLoaderSimple.input && info.CheckpointLoaderSimple.input.required && info.CheckpointLoaderSimple.input.required.ckpt_name) {
                    ...
                    if (Array.isArray(ckptList)) {
                        ckptList.forEach(function(name: string) {
                            var opt = document.createElement('option');
                            opt.value = name;
                            opt.textContent = name;
    ...
    function updateUIForArch(arch: string) {
        ...
        if (archBadge) {
            var archNames: Record<string, string> = { flux: 'FLUX', sdxl: 'SDXL', sd3: 'SD3', sd15: 'SD1.5', ltxv: 'LTX-V', wan: 'WAN', klein: 'KLEIN' };
        ...
        var hs = document.getElementById('gen-height-slider') as HTMLInputElement | null;
        if (isVideo) {
            if (els.customWidth) { els.customWidth.min = '64'; els.customWidth.max = '1280'; els.customWidth.step = '32'; }
            if (els.customHeight) { els.customHeight.min = '64'; els.customHeight.max = '1280'; els.customHeight.step = '32'; }
            if (ws) { ws.min = '64'; ws.max = '1280'; ws.step = '32'; }
            if (hs) { hs.min = '64'; hs.max = '1280'; hs.step = '32'; }
        } else {
            if (els.customWidth) { els.customWidth.min = '256'; els.customWidth.max = '4096'; els.customWidth.step = '64'; }
            if (els.customHeight) { els.customHeight.min = '256'; els.customHeight.max = '4096'; els.customHeight.step = '64'; }
        ...
        if (isVideo) {
            ...
            if (els.cfg) els.cfg.value = '3.0';
            if (els.cfgRange) els.cfgRange.value = '3.0';
            updateDurationHint();
        }
    }
    ...
    function renderLoraList() {
        ...
        list!.querySelectorAll('.gen-lora-strength').forEach(function(slider, idx) {
            slider.addEventListener('input', function(this: HTMLInputElement) {
                ...
                var valInput = this.parentElement!.querySelector('.gen-lora-val-input') as HTMLInputElement | null;
        ...
        list.querySelectorAll('.gen-lora-val-input').forEach(function(input, idx) {
            input.addEventListener('change', function(this: HTMLInputElement) {
                ...
                var slider = this.parentElement!.querySelector('.gen-lora-strength') as HTMLInputElement | null;
                if (slider) slider.value = String(Math.max(-1, Math.min(2, v))); // clamp slider visual
            });
        });
    ...
    function addToGallery(src: string, isVideo: boolean, metadata?: GenerationMetadata) {
        var item = {
            src: src,
            isVideo: !!isVideo,
    ...
    function createThumb(item: GalleryItem, galleryIndex: number) {
        var src = item.src;
        ...
        // Star click
        wrap.querySelector('.gen-thumb-star')!.addEventListener('click', function(this: HTMLElement, e) {
            e.stopPropagation();
    ...
    function navigatePromptHistory(direction: number) {
    ...
    function bindModelPicker() {
        ...
        document.addEventListener('click', function(e) {
            if (els.modelPickerWrap && !els.modelPickerWrap.contains(e.target as Node)) {
                closeModelDropdown();
            }
        });
    }
    ...
    function renderModelDropdown() {
        ...
        var groups: Record<string, ModelUtilEntry[]> = {};
        var groupOrder = ['flux', 'sdxl', 'sd3', 'zimage', 'qwen', 'klein', 'hunyuan', 'sd15', 'ltxv', 'wan', 'other'];
        var groupLabels: Record<string, string> = { flux: 'FLUX', sdxl: 'SDXL', sd3: 'SD3', zimage: 'Z-IMAGE', qwen: 'QWEN', klein: 'KLEIN', hunyuan: 'HUNYUAN', sd15: 'SD 1.5', ltxv: 'Video (LTX)', wan: 'Video (WAN)', other: 'Other' };

        filtered.forEach(function(m: ModelUtilEntry) {
        ...
        if (filtered.length === 0) {
            ...
            groupOrder.forEach(function(arch) {
                ...
                groups[arch].forEach(function(m: ModelUtilEntry) {
    ...
    // ── Prompt Attention Weight (Ctrl+Up/Down) ──
    function handlePromptWeight(textarea: HTMLTextAreaElement, increase: boolean) {
    ...
    function renderGallery() {
        ...
        var prevBtn = document.getElementById('gen-page-prev') as HTMLButtonElement | null;
        var nextBtn = document.getElementById('gen-page-next') as HTMLButtonElement | null;
    ...
    function handleThumbClick(galleryIndex: number, e: MouseEvent) {
    ...
    function showContextMenu(e: MouseEvent, galleryIndex: number) {
        ...
        var item = state.gallery[galleryIndex] || {} as GalleryItem;
    ...
    function handleContextAction(action: string, idx: number) {
        ...
        switch (action) {
            ...
            case 'use-prompt':
                if (item.prompt && els.prompt) {
                    (els.prompt as HTMLTextAreaElement).value = item.prompt;
                    state.prompt = item.prompt;
                    (els.prompt as HTMLTextAreaElement).style.height = 'auto';
                    (els.prompt as HTMLTextAreaElement).style.height = (els.prompt as HTMLTextAreaElement).scrollHeight + 'px';
                    updateTokenCount();
                }
                break;
            case 'use-seed':
                if (item.seed != null && els.seed) {
                    ...
                    state.seed = item.seed as number;
            ...
            case 'use-all':
                ...
                if (item.steps && els.steps) {
                    ...
                    if (els.stepsRange) els.stepsRange.value = String(item.steps);
                    state.steps = item.steps;
                }
                if (item.cfg && els.cfg) {
                    els.cfg.value = String(item.cfg);
                    if (els.cfgRange) els.cfgRange.value = String(item.cfg);
                    state.cfg = item.cfg;
                }
                if (item.guidance && els.guidance) {
                    els.guidance.value = String(item.guidance);
                    if (els.guidanceRange) els.guidanceRange.value = String(item.guidance);
    ...
    function bindGalleryUpload() {
        ...
        var uploadInput = document.getElementById('gen-gallery-upload-input') as HTMLInputElement | null;
        if (uploadBtn && uploadInput) {
            uploadBtn.addEventListener('click', function() { uploadInput!.click(); });
            uploadInput.addEventListener('change', function(this: HTMLInputElement) {
            ...
            fetch('/upload/image', { method: 'POST', body: formData })
                ...
                .catch(function(err) {
                    ...
                    reader.onload = function(ev) {
                        addToGallery(ev.target!.result as string, false, { prompt: '(uploaded)' });
                        renderGallery();
                    };
    ...
    function bindGallerySettings() {
        ...
        // Close on outside click
        document.addEventListener('click', function(e) {
            if (popover && !popover.contains(e.target as Node) && e.target !== settingsBtn) {
        ...
        // Thumb size slider
        var thumbSlider = document.getElementById('gen-thumb-size-slider') as HTMLInputElement | null;
        var thumbVal = document.getElementById('gen-thumb-size-val');
        if (thumbSlider) {
            thumbSlider.value = String(state.thumbSize);
            if (thumbVal) thumbVal.textContent = String(state.thumbSize);
            thumbSlider.addEventListener('input', function(this: HTMLInputElement) {
                state.thumbSize = parseInt(this.value);
                if (thumbVal) thumbVal.textContent = String(state.thumbSize);
                applyThumbSize();
                try { localStorage.setItem('sf-thumb-size', String(state.thumbSize)); } catch(e) {}
            });
        }
    ...
    return {
        ...
        init: init,
        generate: generate,
        displayResult: function(src: string, isVideo: boolean) {
            if (!initialized) init();
