...
code changes::
    ...
    # After:
    from .compat.ramtorch import replace_linear_with_ramtorch, move_model_to_device

The shim wraps the model with :func:`stagehand.layer` internally
and sets ``is_ramtorch = True`` flags on managed modules/params so that
...
def replace_linear_with_ramtorch(
    ...
    *,
    ...
    Internally wraps *module* with :func:`stagehand.layer` and marks
    managed modules/params with ``is_ramtorch = True`` for downstream compat checks.
